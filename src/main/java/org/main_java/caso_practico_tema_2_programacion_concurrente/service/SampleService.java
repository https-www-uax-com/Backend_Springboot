...
public class SampleService {
    ...
    private SampleDTO mapToDTO(final Sample sample, final SampleDTO sampleDTO) {
        ...
        // Asignamos el código de la muestra [SampleCode] de la entidad Sample al DTO
        sampleDTO.setSampleCode(sample.getSampleCode());

        // Asignamos el ID del experimento relacionado [Experiment] al DTO
        sampleDTO.setExperimentId(sample.getExperiment().getId());

        // Creación de un nuevo DTO para BiologicalData y mapeo de sus propiedades
        BiologicalDataDTO biologicalDataDTO = new BiologicalDataDTO();

        // Asignamos el ID del BiologicalData relacionado al DTO
        biologicalDataDTO.setId(sample.getBiologicalData().getId());

        // Asignamos el tipo de muestra [SampleType] de BiologicalData al DTO
        biologicalDataDTO.setSampleType(sample.getBiologicalData().getSampleType());

        // Asignamos los datos específicos de la muestra al DTO
        biologicalDataDTO.setData(sample.getBiologicalData().getData());

        // Asignamos el timestamp del BiologicalData al DTO
    ...
    private Sample mapToEntity(final SampleDTO sampleDTO, final Sample sample) {
        // Asignamos el codigo de la muestra desde el DTO a la entidad Sample
        sample.setSampleCode(sampleDTO.getSampleCode());

        // Si el experimentId esta presente en el DTO, buscamos el Experiment relacionado
        final Experiment experiment = sampleDTO.getExperimentId() == null
                // Si no hay experimentId en el DTO, establecemos experiment como null
                ? null
        ...
        // Si los datos biologicos estan presentes en el DTO, los mapeamos
        final BiologicalData biologicalData = sampleDTO.getBiologicalData() == null
                // Si no hay BiologicalData en el DTO, establecemos biologicalData como null
                ? null
